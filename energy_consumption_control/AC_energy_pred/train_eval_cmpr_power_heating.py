...
from dataset.dataset_cmpr_power import CmprPowerDatasetHeating, recover_data
from draw_pic import draw_pred
from model.model_cmpr_power_heating import MLPModel
...
def train_net(net, train_epoch, train_loader, optimizer, device):
    ...
    for epoch_index in iterator:
        for batch_idx, (input_data, y) in enumerate(train_loader):
            ...
            print(f"train{input_data.shape}->{y.shape}")
            print(f"input_data{input_data[0:10, :]}")
            print(f"output_data{y[0:10, :]}")
            '''
            ...
            pred_cmpr_power = net(input_data)
            print(f"pred_cmpr_power{pred_cmpr_power[0:10, :]}")
            cmpr_power = y[:, 0]
            loss = criterion(pred_cmpr_power, cmpr_power)
            optimizer.zero_grad()
            loss.backward()
...
def eval_net(net, eval_loader, result_pic_folder, device):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(eval_loader):
            ...
            for split_index in range(split_num):
                ...
                pred_cmpr_power = net(split_input)
                cmpr_power = split_y[:, 0]

                final_pred_p = recover_data(pred_cmpr_power, eval_loader.dataset.compressor_p_max, eval_loader.dataset.compressor_p_min)
                real_cmpr_power = recover_data(cmpr_power, eval_loader.dataset.compressor_p_max, eval_loader.dataset.compressor_p_min)

                cmpr_power_mae = torch.mean(torch.abs(real_cmpr_power - final_pred_p))
                # 可视化
                series_list = [
                    # [pred_t_r_1, refrigerant_mix_temp],
                    [final_pred_p, real_cmpr_power]
                ]
                series_name_list = [
                    # ['pred_refrigerant_mix_temp', 'refrigerant_mix_temp'],
                    ['pred_cmpr_power', 'cmpr_power']
                ]
                ...
                all_cmpr_power_mae.append(cmpr_power_mae.view(1, -1))
    ...
    all_cmpr_power_mae = torch.cat(all_cmpr_power_mae)

    # mean_refrigerant_mix_temp_mae = torch.mean(all_refrigerant_mix_temp_mae)
    mae_all_cmpr_power_mae = torch.mean(all_cmpr_power_mae)
    return mae_all_cmpr_power_mae
...
if __name__ == '__main__':
    ...
    pretrain_model_name = 'cmpr_power_net_dnn.pth'
    saved_model_name = 'cmpr_power_net_dnn.pth'
    ...
    load_dataset = False
    train_dataset_path = './data/cmpr_power_train_dataset.pkl'
    eval_dataset_path = './data/cmpr_power_eval_dataset.pkl'
    all_dataset_path = './data/cmpr_power_all_dataset.pkl'
    if load_dataset:
        ...
        train_dataset = CmprPowerDatasetHeating(train_data_path, True)
        eval_dataset = CmprPowerDatasetHeating(eval_data_path, False)
        all_dataset = CmprPowerDatasetHeating(all_data_path, False)
    ...
    load_pretrain = False
    input_dim = 6
    net = MLPModel(input_dim=input_dim)
    if load_pretrain:
    ...
    mean_cmpr_power_mae = eval_net(net, eval_loader, result_pic_folder, device)

    # print('mean_refrigerant_mix_temp_mae', mean_refrigerant_mix_temp_mae)
    print('mean_cmpr_power_mae', mean_cmpr_power_mae)
