...
from dataset.dataset_cab_pos import CabPosBaseDataset
from draw_pic import draw_pred
# from model.model_cmpr_control_tcn import *
from model.model_cab_pos_PID import *
...
def my_args():
    ...
    # 训练数据路径
    parser.add_argument('--train_dataset_path', default='./data/energy_cunsum_data/pkl_file/acc_pid_out/seed2_cobpos_train_filter1022_split.pkl', type=str, help='')
    # 评测数据路径
    parser.add_argument('--eval_dataset_path', default='./data/energy_cunsum_data/pkl_file/acc_pid_out/seed2_cobpos_eval_filter1022_split.pkl', type=str, help='')
    # 总体据路径
    parser.add_argument('--all_dataset_path', default='./data/energy_cunsum_data/pkl_file/acc_pid_out/seed2_cobpos_all_filter1018.pkl', type=str, help='')
    ...
    # 预训练模型路径
    parser.add_argument('--pretrain_ckpt_path', default='./data/ckpt/cp_1022_v2.pth', type=str, help='')
    # 模型保存路径
    parser.add_argument('--saved_ckpt_path', default='data/ckpt/cp_1022_v2.pth', type=str, help='')
    ...
    # 保存图片路径
    parser.add_argument('--result_pic_folder', default='./data/train_result/pic/cp_1022_v2', type=str, help='')
    # 训练时自动保存的测试损失
    parser.add_argument('--best_loss', default=12, type=int, help='')
        ...
        for batch_idx, (input_data, y) in enumerate(train_loader):
            ...
            for split_index in range(split_num):
                ...
                for i in range(split_len):
                    if i % 30 == 0:
                        net.last_cab_pos = split_input[i, 0].unsqueeze(0)
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(eval_loader):
            ...
            for split_index in range(split_num):
                ...
                for i in range(split_len):
                    if i == 0:
                        net.last_cab_pos = split_y[0,0]
                    elif i%120 == 0:
                        net.last_cab_pos = split_y[i-1,0]
                    # net.Delta = None
                    # pred_output[i] = net.last_cab_pos
                    pred_output[i] = net(split_input[i])
...
if __name__ == '__main__':
    ...
    if load_dataset:
        ...
        train_dataset = CabPosBaseDataset(train_data_path, False)
        eval_dataset = CabPosBaseDataset(eval_data_path, False)
        # all_dataset = CabPosBaseDataset(all_data_path, False)
