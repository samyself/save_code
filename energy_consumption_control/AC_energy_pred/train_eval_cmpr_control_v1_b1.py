...
from model.model_cmpr_control2_v2 import MLPModel
from model.model_cmpr_control import MyModel as last_model
...
def my_args():
    ...
    # 训练数据路径
    parser.add_argument('--train_dataset_path', default='./data/cmpr_control_train_dataset_full_seed2.pkl', type=str,
                        help='')
    # 评测数据路径
    parser.add_argument('--eval_dataset_path', default='./data/cmpr_control_eval_dataset_full_seed2.pkl', type=str,
                        help='')
    # 总体据路径
    parser.add_argument('--all_dataset_path', default='./data/cmpr_control_all_dataset_full_seed2.pkl', type=str,
                        help='')
    ...
    # 预训练模型路径
    parser.add_argument('--pretrain_ckpt_path', default='./data/ckpt/cc_1009_sub3.pth', type=str, help='')
    # 模型保存路径
    parser.add_argument('--saved_ckpt_path', default='data/ckpt/cc_1009_sub3.pth', type=str, help='')
    ...
    # 保存图片路径
    parser.add_argument('--result_pic_folder', default='./data/train_result/pic/submits1009_v3', type=str, help='')
    # 训练时自动保存的测试损失
    parser.add_argument('--best_loss', default=400, type=int, help='')
    #是否加载过去模型的部分参数 1：加载转速模型  2：加载开度模型  3：加载所有模型 其他：不加载
    parser.add_argument('--Flag_load', default=1, type=int, help='')
    # 加载模型的路径
    parser.add_argument('--load_ckpt_path', default='./data/ckpt/cc_0927_sub.pth', type=str, help='')
...
def load_prefer_model(prefer_model_path, model, flag, device):
    # 如果路径存在
    if os.path.exists(prefer_model_path):
        prefer_model = last_model()
        prefer_model.load_state_dict(torch.load(prefer_model_path, map_location=torch.device('cpu')))
    else:
        print("模型不存在")
        return None
    if flag == 1:
        model.compressor_speed_model.load_state_dict(prefer_model.compressor_speed_model.state_dict())
    elif flag == 2:
        model.cab_pos_model.load_state_dict(prefer_model.cab_pos_model.state_dict()).to(device)
    elif flag == 3:
        model.load_state_dict(prefer_model.state_dict())
    else:
        print("do not load")
...
def eval_net(net, eval_loader, result_pic_folder, device):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(eval_loader):
            ...
            for split_index in range(split_num):
                ...
                compressor_speed_mae_prob = torch.abs(compressor_speed_pred - compressor_speed_real) / torch.clamp(
                    compressor_speed_real, min=1)
                cab_heating_status_act_pos_mae = torch.mean(
                    torch.abs(cab_heating_status_act_pos_pred - cab_heating_status_act_pos_real))
                cab_heating_status_act_pos_mae_prob = torch.abs(
                    cab_heating_status_act_pos_pred - cab_heating_status_act_pos_real) / torch.clamp(
                    cab_heating_status_act_pos_real, min=1)

                compressor_speed_r = np.corrcoef(compressor_speed_pred, compressor_speed_real)[0][1]
                cab_heating_status_act_pos_r = \
                np.corrcoef(cab_heating_status_act_pos_pred, cab_heating_status_act_pos_real)[0][1]
                ...
                # print(pic_name)
                print('file_name', file_name, 'compressor_speed_mae', compressor_speed_mae,
                      'cab_heating_status_act_pos_mae',
                      cab_heating_status_act_pos_mae, 'compressor_speed_r', compressor_speed_r,
                      'cab_heating_status_act_pos_r',
                      cab_heating_status_act_pos_r)
    ...
    print('cab_heating_status_act_pos_r_mean',
          sum(cab_heating_status_act_pos_r_list) / len(cab_heating_status_act_pos_r_list))
...
if __name__ == '__main__':
    ...
    net = MLPModel(input_size=input_dim, output_dim=output_dim, num_channels=[128] * 2, kernel_size=5, dropout=0.5)
    ...
    load_prefer_model(args.load_ckpt_path,net,args.Flag_load,device)
    # torch.save(net.state_dict(), saved_ckpt_path)
