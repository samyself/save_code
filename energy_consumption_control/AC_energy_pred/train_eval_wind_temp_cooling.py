...
from dataset import WindTempDatasetcooling
from draw_pic import draw_pred
from model import MLPModel, WindTempModel_heat_phsyics, WindTempModel_cool_phsyics
...
def train_net(net, train_epoch, train_loader, optimizer, device):
    ...
    for epoch_index in iterator:
        for batch_idx, (input_data, y) in enumerate(train_loader):
            ...
            refrigerant_mix_temp = input_data[:, 2]
            refrigerant_air_temp = input_data[:, 3]
            cab_cooling_status_act_pos = input_data[:, 4]
            battery_cooling_status_act_pos = input_data[:, 5]

            pred_t_r, pred_t_w = net(refrigerant_mix_temp=refrigerant_mix_temp,
                                     refrigerant_air_temp=refrigerant_air_temp,
            ...
            temp_p_h_1_cab_cooling = y[:, 1]

            # loss_t_r_1 = criterion(pred_t_r_1, refrigerant_mix_temp)
            loss_t_r = criterion(pred_t_r, temp_p_h_1_cab_cooling)
            loss_t_w = criterion(pred_t_w, air_temp_after_heat_exchange)

            loss = loss_t_r + loss_t_w
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(eval_loader):
            ...
            for split_index in range(split_num):
                ...
                refrigerant_mix_temp = split_input[:, 2]
                refrigerant_air_temp = split_input[:, 3]
                cab_cooling_status_act_pos = split_input[:, 4]
                battery_cooling_status_act_pos = split_input[:, 5]
                ...
                temp_p_h_1_cab_cooling = split_y[:, 1]

                # refrigerant_mix_temp_mae = torch.mean(torch.abs(pred_t_r_1 - refrigerant_mix_temp))
                refrigerant_after_heat_exchange_mae = torch.mean(torch.abs(pred_t_r - temp_p_h_1_cab_cooling))
                air_temp_mae = torch.mean(torch.abs(pred_t_w - air_temp_after_heat_exchange))
                ...
                series_list = [
                    ...
                    [pred_t_r, temp_p_h_1_cab_cooling],
                    [pred_t_w, air_temp_after_heat_exchange],
                ]
                series_name_list = [
...
if __name__ == '__main__':
    ...
    pretrain_model_name = 'wind_temp_net_phsyics_cooling.pth'
    saved_model_name = 'wind_temp_net_phsyics_cooling.pth'
    ...
    load_dataset = False
    train_dataset_path = './data/wind_temp_cooling_train_dataset.pkl'
    eval_dataset_path = './data/wind_temp_cooling_eval_dataset.pkl'
    all_dataset_path = './data/wind_temp_cooling_all_dataset.pkl'
    if load_dataset:
        ...
        train_dataset = WindTempDatasetcooling(train_data_path, True)
        eval_dataset = WindTempDatasetcooling(eval_data_path, False)
        all_dataset = WindTempDatasetcooling(all_data_path, False)
    ...
    # 网络
    load_pretrain = True
    net = WindTempModel_cool_phsyics()
    if load_pretrain:
