...
def my_args():
    ...
    # 训练数据路径
    parser.add_argument('--train_dataset_path', default='./data/energy_cunsum_pkl/cmpr_control_train_dataset_full_seed2_filter.pkl', type=str, help='')
    # 评测数据路径
    parser.add_argument('--eval_dataset_path', default='./data/energy_cunsum_pkl/cmpr_control_eval_dataset_full_seed2_filter.pkl', type=str, help='')
    # 总体据路径
    parser.add_argument('--all_dataset_path', default='./data/energy_cunsum_pkl/cmpr_control_all_dataset_full_seed2_filter.pkl', type=str, help='')
    ...
    # 保存图片路径
    parser.add_argument('--result_pic_folder', default='./data/train_result/pic/cc_0927_sub_112', type=str, help='')
    # 训练时自动保存的测试损失
...
if __name__ == '__main__':
    ...
    net = MLPModel(1, 1, 1,1, dropout=0.5)

    if load_pretrain:
        if os.path.exists(pretrain_ckpt_path):
            # net1 = torch.load(pretrain_ckpt_path, map_location=torch.device('cpu'))
            net.load_state_dict(torch.load(pretrain_ckpt_path, map_location=torch.device('cpu')).state_dict())
    net.to(device)
    # torch.save(net.state_dict(), saved_ckpt_path)
