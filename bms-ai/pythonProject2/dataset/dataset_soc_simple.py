...
def data_prepare1(T):
    ...
    T = T.iloc[idx_begin - 600:, :].reset_index(drop=True)
...
def merge_data(file_path_list, time_step, time_slip, time_skip, return_point):
    ...
    for file_path in tqdm(file_path_list):
        ...
        df_origin = df_origin.fillna(method='ffill').fillna(method='bfill')
        df = data_prepare1(df_origin)
        time_pred = 2
...
class SOCBaseDataset(torch.utils.data.Dataset):
    def __init__(self, all_data_path, time_step=600, time_slip=1, time_skip=1, return_point=True):
