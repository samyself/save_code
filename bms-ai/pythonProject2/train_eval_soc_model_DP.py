...
from dataset.dataset_soc_simple_1_5 import SOCBaseDataset
...
from model.model_soc_lstmv1_delta import MyModel
...
def my_args():
    ...
    # 预训练模型路径
    parser.add_argument('--pretrain_ckpt_path', default='../data/ckpt/Soc_0102_lstm_v1_2_delta.pth', type=str, help='')
    # 模型保存路径
    parser.add_argument('--saved_ckpt_path', default='../data/ckpt/Soc_0102_lstm_v1_2_delta.pth', type=str, help='')
    # 是否训练
    parser.add_argument('--Flag_train', default=False, type=bool, help='')
    # 保存图片路径
    parser.add_argument('--result_pic_folder', default='../data/pic/Soc_0102_lstm_v1_2_delta_0124badcase', type=str,
                        help='')
    # 保存csv路径
    parser.add_argument('--result_csv_folder', default='../data/csv_res/Soc_0102_lstm_v1_2_delta_0124badcase', type=str,
                        help='')
    # 训练时自动保存的测试损失
    parser.add_argument('--best_loss', default=-0.001, type=int, help='')
...
def train_net(net, train_epoch, train_loader, eval_loader, optimizer, device):
    ...
    for epoch_index in iterator:
        ...
        for batch_idx, (input_data, y) in enumerate(train_loader):
            ...
            # loss = get_healthy_loss(pred_out12, delta, y[:,0])
            # print('train_loss',loss)
            optimizer.zero_grad()
            loss.backward()
            # net.module.SocPreModel.load_state_dict(torch.load(soc_pre_model_path, map_location=torch.device(device)))
            # net.module.SocPreModel.load_state_dict(net1.SocPreModel.state_dict())
            # net.module.SocPreModel.load_state_dict(torch.load(soc_pre_model_path))
        ...
        if eval_loss[-1] + train_loss[-1] < A:
            A = eval_loss[-1] + train_loss[-1]
        ...
        for batch in eval_loader:
            ...
            for split_index in range(split_num):
                if len(input_data.shape) != 4:
                    ...
                    continue
                split_input = input_data.reshape(-1, input_data.shape[-3], input_data.shape[-2], input_data.shape[-1])[
                              0, :, :, :].to(device)
...
if __name__ == '__main__':
    ...
    if load_dataset:
        ...
        data_folder = '../data/filter_data/badcase_filter'
    ...
    # soc_pre_model_path = '../data/ckpt/Soc_0102_lstm_tcn_v1_3.pth'
    # net1.load_state_dict(torch.load(soc_pre_model_path, map_location=torch.device('cpu')))

    # net.SocPreModel.load_state_dict(net1.SocPreModel.state_dict())
    # net.SocPreModel.load_state_dict(torch.load(soc_pre_model_path))

    net.to(device).float()
    print('model in ', device)

    net_use = nn.DataParallel(net, device_ids=device_ids)
    # net = net_use.module
    ...
    if Flag_train:
        optimizer = torch.optim.Adam(net_use.module.SocDeltaModel.parameters(),
                                     lr=lr,
                                     weight_decay=weight_decay)

        # for param in net_use.module.SocPreModel.parameters():
    ...
    # 测试
    # if not os.path.exists(saved_ckpt_path):
    #     torch.save(net_use.module.state_dict(), saved_ckpt_path)
    # net_use.module.load_state_dict(torch.load(saved_ckpt_path, map_location=torch.device('cuda')))
