...
def analyze_csv(csv_path1, csv_path2, save_csv_folder_path):
    # 从CSV文件中加载DataFrame
    ...
    # df = df[mask]
    df = df1.copy()
    A1 = df1['Soc_pred'].values +1
    A2 = df1['unhealthy_value'].values +1
    B1 = df2['Soc_pred'].values
    B2 = df2['unhealthy_value'].values

    df['Soc_pred'] = (A2*B1+B2*A1)/(A2+B2)
    # 取A2和B2的最大值作为unhealthy_value
    df['unhealthy_value'] = np.maximum(A2, B2)

    #保存csv
    df.to_csv(save_csv_folder_path, index=False)
    ...
    for i in range(len(analyze_info_list) - 1):
        ...
        file_name = os.path.basename(csv_path1).replace('.csv', '').split('/')[-1].split('\\')[-1]
...
if __name__ == '__main__':
    intput_csv_folder1 = '../../data/csv_res/Soc_1213_lstm_v1_h'
    intput_csv_folder2 = '../../data/csv_res/Soc_1213_lstm_v1_h'
    save_csv_folder = '../../data/csv_res/Soc_1213_lstm_v1_h_and_Soc_1213_lstm_v1_h'

    json_path = '../../data/json_res/analyze_info_Soc_1213_lstm_v1_h_and_Soc_1213_lstm_v1_h.json'
    all_path_list = os.listdir(intput_csv_folder1)
    ...
    with open(json_path, 'w') as f:
        ...
        for i in tqdm(range(len(all_path_list))):
            ...
            if '.csv' in path:
                ...
                continue
            csv_path1 = os.path.join(intput_csv_folder1, path)
            csv_path2 = os.path.join(intput_csv_folder2, path)
            save_csv_folder_path = os.path.join(save_csv_folder, path)
            output_dict, Soc_pr_erro = analyze_csv(csv_path1,csv_path2,save_csv_folder_path)
