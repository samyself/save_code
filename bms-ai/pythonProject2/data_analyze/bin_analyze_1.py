...
def calculate_gamma_bin(all_y_true, all_y_pred, all_gamma):
    ...
    print('len all_y_true', len(all_y_true))
    ...
    for label, info in mae_by_bin.items():
        print(
            f"Bin {label}: Range = {info['Range']},Mae = {info['MAE']:.4f}, Std= {info['Std']:.4f}, Max= {info['Max']:.4f} ,Count = {info['count']:.4f},Count_err = {info['count_err']:.4f}")
...
if __name__ == '__main__':
    ...
    for file in csv_files:
        ...
        # 只统计80~100的部分
        mask = data[f'Soc_real'].values >= 80
        data = data[mask]
