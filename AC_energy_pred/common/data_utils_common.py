...
# 出风模式
def get_hvac_mode(tem_df, car_type):
    if car_type == 'modena':
        hvac_mode = tem_df['TmRteOut_HvacFirstLeWindModSt']
        hvac_mode_first = hvac_mode
        hvac_mode_second = hvac_mode
        # tem_df['hvac_mode'] = hvac_mode
        # tem_df['hvac_mode_first'] = hvac_mode
        # tem_df['hvac_mode_second'] = hvac_mode

    elif 'lemans' in car_type:

        # 前排模式
        if 'Outlet_SysN_FL' in tem_df:
            hvac_mode_first_inner = np.array(tem_df['Outlet_SysN_FL'])
            hvac_mode_first = []
            for time_index in range(len(hvac_mode_first_inner)):
                now_hvac_mode_first_inner = np.floor(hvac_mode_first_inner[time_index])
                now_hvac_mode_first = config_common.mode_inner_to_can_first[now_hvac_mode_first_inner]
                hvac_mode_first.append(now_hvac_mode_first)
            hvac_mode_first = np.array(hvac_mode_first)
        elif 'TmRteOut_HvacFirstLeWindModSt' in tem_df:
            hvac_mode_first = np.array(tem_df['TmRteOut_HvacFirstLeWindModSt'])
        elif 'SubCANR_HvacFirstLeWindModSt' in tem_df:
            hvac_mode_first = np.array(tem_df['SubCANR_HvacFirstLeWindModSt'])

        # 后排模式
        if 'Outlet_SysN_RL' in tem_df:
            hvac_mode_second_inner = np.array(tem_df['Outlet_SysN_RL'])
            hvac_mode_second = []
            for time_index in range(len(hvac_mode_second_inner)):
                now_hvac_mode_second_inner = np.floor(hvac_mode_second_inner[time_index])
                now_hvac_mode_second = config_common.mode_inner_to_can_second[now_hvac_mode_second_inner]
                hvac_mode_second.append(now_hvac_mode_second)
            hvac_mode_second = np.array(hvac_mode_second)
        elif 'SubCANR_HvacReLeWindModSt' in tem_df:
            hvac_mode_second = np.array(tem_df['SubCANR_HvacReLeWindModSt'])
        # hvac_mode_first = np.ones_like(hvac_pwm) * 2
        # hvac_mode_second = np.ones_like(hvac_pwm) * 2

        # try:
        #     is_second_open = np.array(tem_df['SubCANR_HvacReOnOffSt'])
        #     hvac_mode_second[is_second_open == 0] = 0
        # except Exception as e:
        #     pass

        # 挡位
        auto_gear_first = np.array(tem_df['HMI_AutoLv_N'])
        hand_gear_first = np.array(tem_df['HMI_Blower_FL'])
        auto_mode = np.array(tem_df['auto_mode'])
        # 自动的部分用自动
        gear_first = auto_gear_first * auto_mode + hand_gear_first * (1 - auto_mode)

        gear_second = np.array(tem_df['HMI_Blower_RL'])

        hvac_mode = np.array([hvac_mode_first, hvac_mode_second, gear_first, gear_second, auto_mode]).T
        # tem_df['hvac_mode_first'] = hvac_mode_first
        # tem_df['hvac_mode_second'] = hvac_mode_second
    return hvac_mode, hvac_mode_first, hvac_mode_second
...
def cal_air_temp_after_heat_exchange(tem_df, air_temp_before_heat_exchange, mix_door_pwm_dict, all_vol_rate_channel):
    ...
    for mix_door_pwm_name in config_common.mix_door_control:
        ...
        for name_index in range(len(all_controlled_wind_channel_temp_name)):
            ...
            use_before_rate_heat = (max_mix_door_pwm - mix_door_pwm) / (mix_door_pwm + 1e-9)
            wind_channel_temp_before_mix_door[not_use_before] = wind_channel_temp[not_use_before] * max_mix_door_pwm / mix_door_pwm[not_use_before] - \
                                                                air_temp_before_heat_exchange[not_use_before] * use_before_rate_heat[not_use_before]
            all_wind_channel_before_mix_door_dict[f'{controlled_wind_channel_temp_name}_before'] = wind_channel_temp_before_mix_door
            # if (wind_channel_temp_before_mix_door < -30).any():
            #     print()
    ...
    for time_index in range(len(all_wind_channel_temp)):
        ...
        # if (now_air_temp_after_heat_exchange < -30):
        #     print()
...
# 获取风量比例
def get_vol_rate(tem_df, car_type, all_vol_rate):
    try:
        if car_type == 'modena':
            for temp_id in range(config_cab.outlet_num_modena):
                tem_df[f'vol_rate_{temp_id}'] = all_vol_rate[:, temp_id]

            # 计算出风口温度
            for temp_name in config_cab.all_wind_temp_outlet_dict_modena:
                used_key = config_cab.all_wind_temp_outlet_dict_modena[temp_name]
                all_data = []
                for k_index in range(len(used_key)):
                    k = used_key[k_index]
                    data = tem_df[k]
                    all_data.append(data)

                all_data = np.array(all_data)
                temp = np.mean(all_data, axis=-0)
                tem_df[temp_name] = temp
        elif 'lemans' in car_type:
            for temp_id in range(config_cab.outlet_num_lemans):
                tem_df[f'vol_rate_{temp_id}'] = all_vol_rate[:, temp_id]

            # 计算出风口温度
            try:
                for temp_name in config_cab.all_wind_temp_outlet_dict_lemans:
                    used_key = config_cab.all_wind_temp_outlet_dict_lemans[temp_name]
                    all_data = []
                    for k_index in range(len(used_key)):
                        k = used_key[k_index]
                        data = tem_df[k]
                ...
                for temp_name in config_cab.all_wind_temp_outlet_dict_lemans_heihe:
                    used_key = config_cab.all_wind_temp_outlet_dict_lemans_heihe[temp_name]
                    all_data = []
                    for k_index in range(len(used_key)):
                        k = used_key[k_index]
                        data = tem_df[k]
...
# out_frac
def cal_out_frac(tem_df, car_type, lemans_old=False):
    # 外循环比例
    if 'Inlet_LV' in tem_df or 'Inlet_StsOut' in tem_df:
        if 'Inlet_LV' in tem_df:
            inlet_gear = np.array(tem_df['Inlet_LV'])
        elif 'Inlet_StsOut' in tem_df:
            inlet_gear = np.array(tem_df['Inlet_StsOut'])
        if car_type == 'modena':
            out_frac = interp_from_table(inlet_gear, config_common.inlet_gear_to_out_frac_modena[:, 0],
                                         config_common.inlet_gear_to_out_frac_modena[:, 1]) / 100
        elif 'lemans' in car_type:
            if lemans_old:
                out_frac = interp_from_table(inlet_gear, config_common.inlet_gear_to_out_frac_lemans_old[:, 0],
                                             config_common.inlet_gear_to_out_frac_lemans_old[:, 1]) / 100
            else:
                out_frac = interp_from_table(inlet_gear, config_common.inlet_gear_to_out_frac_lemans[:, 0],
                                             config_common.inlet_gear_to_out_frac_lemans[:, 1]) / 100
    else:
        ...
        if car_type == 'modena':
            ...
            if use_dc_place.any():
                if lemans_old:
                    use_dc_place_out_frac = interp_from_table(inlet_pwm[use_dc_place], config_common.inlet_pwm_to_out_frac_lemans_old[:, 0],
                                                              config_common.inlet_pwm_to_out_frac_lemans_old[:, 1]) / 100
                else:
            ...
            if (~use_dc_place).any():
                if lemans_old:
                    use_lin_place_out_frac = interp_from_table(inlet_pwm_d_lin[~use_dc_place], config_common.inlet_pwm_d_lin_to_out_frac_lemans_old[:, 0],
                                                               config_common.inlet_pwm_d_lin_to_out_frac_lemans_old[:, 1]) / 100
                else:
