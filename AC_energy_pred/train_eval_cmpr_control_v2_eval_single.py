...
def my_args():
    ...
    # 训练数据路径
    parser.add_argument('--train_dataset_path', default='./data/cmpr_control_train_dataset_full_seed2.pkl', type=str, help='')
    # 评测数据路径
    parser.add_argument('--eval_dataset_path', default='./data/cmpr_control_eval_dataset_full_seed2.pkl', type=str, help='')
    # 总体据路径
    parser.add_argument('--all_dataset_path', default='./data/cmpr_control_all_dataset_full_seed2.pkl', type=str, help='')

    # 常修改参数
    # 学习率
    parser.add_argument('--lr', default=1e-3, type=float, help='')
    # 训练batch_size
    parser.add_argument('--train_batch_size', default=config.train_batch_size, type=int, help='')
    # 权重衰减系数
    ...
    # 预训练模型路径
    parser.add_argument('--pretrain_ckpt_path', default='./data/ckpt/cc_0927_sub.pth', type=str, help='')
    # 模型保存路径
    parser.add_argument('--saved_ckpt_path', default='data/ckpt/cc_0927_sub.pth', type=str, help='')
    ...
    # 保存图片路径
    parser.add_argument('--result_pic_folder', default='./data/train_result/pic/submits10927', type=str, help='')
    # 训练时自动保存的测试损失
        ...
        for batch in tqdm(eval_loader):
            ...
            for split_index in range(split_num):
                ...
                for i in range(split_len):
                    # 获取真实值
                    pred_output = torch.cat((pred_output, net(split_input[i,:])),dim=0)
...
if __name__ == '__main__':
    ...
    if load_pretrain:
        if os.path.exists(pretrain_ckpt_path):
            ...
            net.load_state_dict(net1.state_dict())
    net.to(device)
