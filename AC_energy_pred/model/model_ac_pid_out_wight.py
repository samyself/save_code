...
from AC_energy_pred.model.model_format import inter2D, inter1D,init_weights_xavier_uniform,MultiHeadAttention
...
class MyModel(nn.Module):
    def __init__(self):
        ...
        self.MLP1 = MLP(input_size=10, hidden_sizes=[512, 512], output_size=1, use_batchnorm=True, use_dropout=True)

        self.SelfAttention = MultiHeadAttention(d_model=6, num_heads=3)
        self.MLP2 = MLP(input_size=12, hidden_sizes=[512, 512], output_size=1, use_batchnorm=True, use_dropout=True)
    ...
    def forward(self, x):
        ...
        use_x1 = torch.cat((temp_amb.unsqueeze(1), cab_fl_set_temp.unsqueeze(1),cab_fr_set_temp.unsqueeze(1),
                            temp_incar.unsqueeze(1),temp_battery_req.unsqueeze(1),ac_kp_rate_last.unsqueeze(1),
                            lo_pressure.unsqueeze(1), hi_pressure.unsqueeze(1),aim_lo_pressure.unsqueeze(1),aim_hi_pressure.unsqueeze(1)), dim=1)
        pid_paras = self.MLP1(use_x1)

        # b*dim
        use_x2 = torch.cat((pid_paras,diff_hi_pressure.unsqueeze(1),lo_pressure.unsqueeze(1),aim_lo_pressure.unsqueeze(1),hi_pressure.unsqueeze(1),aim_hi_pressure.unsqueeze(1),
                            ), dim=1)

        # 输入需要维b*n*dim  输出为b*n*dim
        final_X = self.SelfAttention(use_x2.unsqueeze(1))
        # b*1*dim
        final_X = torch.sum(final_X, dim=1)

        # #b*(dim+dim)
        # final_X = torch.cat((final_X.squeeze(1),use_x2), dim=1)

        ac_pid_out_hp = self.MLP2(final_X)
