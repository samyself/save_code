...
from AC_energy_pred.model.model_format import inter2D, inter1D,init_weights_xavier_uniform,LSTM
...
class MyModel(nn.Module):
    def __init__(self):
        ...
        self.LSTM = LSTM(input_dim=12, output_dim=1,hidden_size=128,num_layers=2)
        # 1*feature
        self.last_use_x = torch.zeros([2, 12])

        # self.MLP = MLP(input_size=13, hidden_sizes=[512, 512, 512], output_size=1, use_batchnorm=True, use_dropout=True)
    ...
    def forward(self, x):
        ...
        #输入为b*t*feature
        use_x_add = use_X.unsqueeze(1)
        use_x_last = use_X
        L = self.last_use_x.shape[0]
        for i in range(L):
            use_x_last = torch.cat((self.last_use_x[i, :].unsqueeze(0), use_x_last), dim=0)
            use_x_last = use_x_last[:-1, :]
            use_x_add = torch.cat((use_x_last.unsqueeze(1), use_x_add), dim=1)

        self.last_use_x = torch.cat((self.last_use_x, use_X), dim=0).detach()
        self.last_use_x = self.last_use_x[L-1:,:]

        ac_pid_out_hp = self.LSTM(use_x_add)
