...
class MyCabPosModel(nn.Module):
    def __init__(self, input_size=6,output_size=1, hidden_size1=None, hidden_size2=None, dropout = 0.2,first_ipid=1.0):
        super(MyCabPosModel,self).__init__()
        # 开度建模

        self.cab_pos_part1 = MLP(input_size=1, hidden_sizes=[512,512], output_size=1, use_batchnorm=False)
        self.cab_pos_part2 = MLP(input_size=1, hidden_sizes=[2, 2], output_size=1, use_batchnorm=False)
        self.cab_pos_part3 = MLP(input_size=5, hidden_sizes=[512,512], output_size=1, use_batchnorm=False)
        ...
        # 模拟ipid
        self.last_ipid_part = torch.tensor([first_ipid])

    def forward(self, SCErr, use_x):
        part1 = self.cab_pos_part1(SCErr.unsqueeze(1))
        part2 = self.cab_pos_part2(self.last_ipid_part.unsqueeze(1))
        part3 = self.cab_pos_part3(use_x)
        cab_heating_status_act_pos = part1 + part2 + part3
        self.last_ipid_part = cab_heating_status_act_pos[:,0].detach()
...
class MyComSpeedModel(nn.Module):
    def __init__(self, input_size=6, output_size=1, hidden_size1=None, hidden_size2=None, hidden_size3=None, dropout = 0.5):
        ...
        self.com_speed_part1 = MLP(input_size=1, hidden_sizes=hidden_size1, output_size=1, use_batchnorm=False)

        self.com_speed_part2 = MLP(input_size=1, hidden_sizes=hidden_size2, output_size=1, use_batchnorm=False)

        self.com_speed_part3 = MLP(input_size=4, hidden_sizes=hidden_size3, output_size=1, use_batchnorm=False)
    ...
    def forward(self, dif_hi_pressure, ac_pid_out_hp, use_x1):
        ...
        compressor_speed = torch.zeros_like(part1)
...
class MyModel(nn.Module):
    def __init__(self):
        ...
        self.cab_pos_model = MyCabPosModel(input_size=6,output_size=1,first_ipid = self.first_ipid)
    ...
    def forward(self, x):
        ...
        cab_heating_status_act_pos = self.cab_pos_model(SCErr, use_x)
