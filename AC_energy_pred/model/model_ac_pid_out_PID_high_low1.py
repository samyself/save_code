...
class MyModel(nn.Module):
    def __init__(self, device='cpu', car_type='modena'):
        ...
        self.diffpress_Kp_list_HP = torch.tensor([-10, -2.5, -1, 1, 2.5, 10], device=device)
        # 高压差-Ki表
        self.PID_Ki_list_HP = torch.tensor([0.1503906, 0.1503906, 0.1503906, 0.1503906, 0.1503906, 0.1503906], device=device)
        self.diffpress_Ki_list_HP = torch.tensor([-10, -2.5, -1, 1, 2.5, 10], device=device)


        # 低压差-Kp表
        self.PID_Kp_list_LP = torch.tensor([0.49, 0.49, 0.05, 0.05, 0.49,0.49], device=device)
        self.diffpress_Kp_list_LP = torch.tensor([-10, -2.5, -1, 1, 2.5, 10], device=device)
        # 高压差-Ki表
        self.PID_Ki_list_LP = torch.tensor([0.15, 0.13, 0.12, 0.06, 0.06, 0.09], device=device)
        self.diffpress_Ki_list_LP = torch.tensor([-10, -2.5, -1, 1, 2.5, 10], device=device)
    ...
    def forward(self, x):
        ...
        if self.last_ac_pid_out_hp is None:
            self.last_ac_pid_out_hp = self.last_AC_RPM_Req

        if self.last_AC_Percent_Req is None:
            self.last_AC_Percent_Req = self.last_AC_RPM_Req
       ...
       #高压偏差
        ...
        last_ac_pid_out_hp = self.last_ac_pid_out_hp.view(-1, 1)
        ...
        # 20250206-1 得到ac_pid_out_hp
        ac_pid_out_hp = last_ac_pid_out_hp + offset
        self.last_ac_pid_out_hp = ac_pid_out_hp
        ...
        # 20250206-1 得到AC_Pecent_Req
        if f_min != -1.0 and f_min < 1.0:
            ac_pid_out_hp = torch.min(ac_pid_out_hp, self.last_AC_Pecent_Req.view(-1, 1))

        else:
            ac_pid_out_hp = torch.min(ac_pid_out_hp, self.last_AC_RPM_Req.view(-1, 1) + 800.0)

        AC_Pecent_Req = ac_pid_out_hp * f
        self.last_AC_Pecent_Req = AC_Pecent_Req
        ...
        AC_RPM_Req = AC_Pecent_Req + offset
        ...
        if AC_RPM_Req < com_speed_min.item() - 120:
            com_speed_min = torch.tensor([800.0])

        # ac_pid_out_hp = last_ac_pid_out_hp + offset
        AC_RPM_Req = torch.clamp(AC_RPM_Req, min=com_speed_min.item(), max=8000.0)
        ...
        if self.last_offset >= 800:
            ...
            AC_RPM_Req = last_ac_pid_out_hp


        return torch.cat((AC_RPM_Req.view(-1,1),Kp_HP.view(-1,1),Ki_HP.view(-1,1),Kd_HP.view(-1,1),Iout_HP.view(-1,1),Diffout_HP.view(-1,1),HP_AD.view(-1,1),
