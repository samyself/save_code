...
class ExvPidIoutBaseDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, all_data_path, return_point):
        ...
        for path_index in tqdm(range(len(self.all_data_path))):
            ...
            # 上一时刻EXV_Oh_PID_Iout
            last_exv_pid_iout = out_dict['exv_pid_iout'][:-1]
            ...
            # 目标过冷度 目标过热度 上一时刻压缩机进气温度
            # 上一时刻EXV_Oh_PID_Iout 当前副驾设定温度 上一时刻饱和高压 上一时刻饱和低压
            x = ([sc_tar_mode_10] + [sh_tar_mode_10] +[temp_p_h_1_cab_heating] +
                 [temp_p_h_5] + [hi_pressure] + [lo_pressure] + [compressor_speed_last] + [last_exv_pid_iout] )
            x = np.array(x).T
            ...
            mask = (hp_mode != config_all.heat_mode_index) | (last_exv_pid_iout < 12)
