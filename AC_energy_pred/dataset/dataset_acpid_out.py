...
class AcPidOutBaseDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, all_data_path, return_point, car_type='lemans_low'):
        ...
        for path_index in tqdm(range(len(self.all_data_path))):
            ...
            out_dict = read_csv(data_path,car_type)

            # 上一时刻ac_pid_pout
            # AC_PID_out
            last_ac_pid_out_hp = out_dict['ac_pid_out_hp'][:-1]
            ...
            # 上一时刻饱和低压
            # LoSideP
            last_lo_pressure = out_dict['lo_pressure'][:-1]
            # 上一时刻饱和高压
            # HiSideP
            last_hi_pressure = out_dict['hi_pressure'][:-1]
            # 上一时刻目标饱和低压
            # RefrigLoPTar
            aim_lo_pressure = out_dict['aim_lo_pressure'][:-1]  # 感觉应该是当前时刻
            # 上一时刻目标饱和高压
            # RefrigHiPTar
            aim_hi_pressure = out_dict['aim_hi_pressure'][:-1]  # 感觉应该是上一时刻

            # 当前乘务舱温度 TmRteOut_FrntIncarTEstimd
            last_temp_incar = out_dict['temp_in_car'][:-1]  # 感觉应该是上一时刻
            # 电池请求冷却液温度
            temp_battery_req = out_dict['temp_battery_req'][1:]
            # 冷却液进入电池的温度
            last_temp_coolant_battery_in = out_dict['temp_coolant_battery_in'][:-1]  # 感觉应该是上一时刻

            # ac_kp_rate
            ac_kp_rate_last = out_dict['ac_kp_rate'][:-1]

            # 压缩机排气温度
            last_temp_p_h_2 = out_dict['temp_p_h_2'][:-1]
            # 内冷温度
            last_temp_p_h_5 = out_dict['temp_p_h_5'][:-1]
            # 压缩机进气温度
            last_temp_p_h_1_cab_heating = out_dict['temp_p_h_1_cab_heating'][:-1]
            # 蒸发器温度
            temp_evap = out_dict['temp_evap'][1:]  # 感觉应该是当前的

            # 空调模式
            hp_mode_ac = out_dict['hp_mode_ac'][1:]  # 感觉应该是当前的
            # 制冷模式下膨胀阀开度
            last_cab_cooling_status_act_pos = out_dict['cab_cooling_status_act_pos'][:-1]
            ...
            compressor_speed_change = abs(compressor_speed - last_compressor_speed)
            ...
            x = ([last_ac_pid_out_hp] +
                 [temp_amb] + [cab_fl_set_temp] + [cab_fr_set_temp] +
                 [last_lo_pressure] + [last_hi_pressure] + [aim_lo_pressure] + [aim_hi_pressure] +
                 [last_temp_incar] + [temp_battery_req] + [last_temp_coolant_battery_in] +
                 [last_temp_p_h_2] + [temp_evap] + [hp_mode_ac] + [last_cab_cooling_status_act_pos])
