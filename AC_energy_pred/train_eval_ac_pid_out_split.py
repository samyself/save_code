...
from model.model_ac_pid_out_PID_train import *
import pandas as pd
...
def my_args():
    ...
    # 预训练模型路径
    parser.add_argument('--pretrain_ckpt_path', default='./data/ckpt/ac_20250120_v2.pth', type=str, help='')
    # 模型保存路径
    parser.add_argument('--saved_ckpt_path', default='data/ckpt/ac_20250120_v2.pth', type=str, help='')
    ...
    # 保存图片路径
    parser.add_argument('--result_pic_folder', default='./data/train_result/pic/ac_20250120_v2', type=str, help='')
    # 训练时自动保存的测试损失
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(eval_loader):
            ...
            for split_index in range(split_num):
                ...
                ac_pid_out_err = torch.abs(ac_pid_out_pred - ac_pid_out_real)

                # if ac_pid_out_err > 100:
                #     print(ac_pid_out_err)

                # ac_pid_out_pred保存到csv
                # 保存到csv
                # pd.DataFrame(ac_pid_out_err.cpu().numpy()).to_csv('D:/Code/code/submit/energy_consumption_control/AC_energy_pred/data/csv_res/badcase_ac_pid_out_err.csv')
                ...
                diff_hi_press = aim_hi_pressure - hi_pressure
                # 可视化
                series_list = [
...
if __name__ == '__main__':
    ...
    if load_dataset:
        ...
        train_dataset = AcPidOutBaseDataset(train_data_path, False)
